...
type ProfessionalLoadingLayoutProps = {
  navItemCount?: number;
};

type ProfessionalDashboardLayoutProps = {
  isMobile: boolean;
  navLinks: any[];
  currentView: string;
...
export function ProfessionalLoadingLayout({ navItemCount = 4 }: ProfessionalLoadingLayoutProps) {
  return <DashboardLoading title="Professional Admin" navItemCount={navItemCount} showSearch={true} />;
}
    ...
    <div className="max-h-screen min-h-screen relative flex justify-center">
      ...
      <div className="flex flex-1 overflow-hidden max-w-[1440px] w-full mx-auto border-r border-l border-gray-200">
        <SharedSidebar
          ...
          isMobile={isMobile}
          headerTitle="Professional Admin"
          headerIcon={Shield}
        />
