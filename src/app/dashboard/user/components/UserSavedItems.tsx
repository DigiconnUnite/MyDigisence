...
interface UserSavedItemsProps {
  savedItems: any[]
}

export default function UserSavedItems({ savedItems }: UserSavedItemsProps) {
  const [activeTab, setActiveTab] = useState<'businesses' | 'professionals'>('businesses')

  const savedBusinesses = savedItems.filter((s) => s.type === 'business')
  const savedProfessionals = savedItems.filter((s) => s.type === 'professional')
